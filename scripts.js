/* -------------------------------------------------------------
   ...
   scripts.js - versão com ICO · Índice de Eficiência Operacional
   -------------------------------------------------------------
   Blocos:
     1. Configuração - constantes e registro de plugins
     2. Cálculo - funções de custo e ROI
     3. Utilitários - formatação e gráficos
     4. Motor ICO - IOA · Inteligência Operacional AFLOR
     5. Atualização de Interface - KPIs e DOM
...
function gerarProximoPasso(tipo) {

  switch (tipo) {

    case 'financeiro_e_operacional':
      return `
        <strong>Validar implantação inicial AFLOR</strong><br><br>
        Validar os fluxos prioritários da operação e estruturar um plano inicial de implantação AFLOR, considerando ganho financeiro, recuperação de capacidade operacional e padronização do fluxo.
      `;

    case 'financeiro_moderado':
      return `
        <strong>Avaliar implantação por ganho financeiro</strong><br><br>
        Mapear os processos com maior custo operacional e avaliar uma implantação direcionada, priorizando redução de custo, padronização e controle dos fluxos já existentes.
      `;

    case 'equilibrio_com_capacidade':
      return `
        <strong>Avaliar troca de custo por capacidade</strong><br><br>
        Avaliar se a recuperação de horas e a redução de sobrecarga justificam manter custo semelhante ao modelo atual, com maior organização, rastreabilidade e capacidade operacional.
      `;

    case 'operacional_sem_economia':
      return `
        <strong>Validar ganho operacional antes da implantação</strong><br><br>
        Identificar quais atividades manuais mais sobrecarregam a equipe e validar se a recuperação de capacidade operacional justifica avançar, mesmo sem economia financeira direta.
      `;

    case 'operacao_suficiente':
      return `
        <strong>Manter acompanhamento do processo atual</strong><br><br>
        Manter o processo atual em acompanhamento e revisitar o diagnóstico caso aumentem o volume de formulários, o tempo improdutivo, a equipe envolvida ou a necessidade de controle operacional.
      `;

    default:
      return `
        <strong>Mapear processos antes de avançar</strong><br><br>
        Realizar uma leitura mais detalhada dos fluxos operacionais antes de decidir pela implantação, priorizando processos repetitivos, gargalos manuais e pontos com maior impacto na rotina da equipe.
      `;
  }
}
...
/**
 * Calcula o ICO · Índice de Eficiência Operacional.
 *
 * Pesos:
...
function calcularICO(economiaMensal, custoManualMensal, horasRecuperadas, colaboradores, payback) {
  ...
  // Capacidade mensal total da equipe em horas
  const capacidadeTotalHoras = colaboradores * HORAS_MES;
  let scoreTempo = 0;
  if (capacidadeTotalHoras > 0 && horasRecuperadas > 0) {
    const proporcaoTempo = horasRecuperadas / capacidadeTotalHoras;
    scoreTempo = Math.min(Math.round(proporcaoTempo * 400), 100);
  }
...
/* -- 5. ATUALIZACAO DE INTERFACE (KPIs) + RELATÓRIO --------- */
...
function calcularROI() {
  ...
  document.getElementById('rp_recText').innerHTML    = gerarProximoPasso(ico.icoTipo);
